...
public class Skupno {
    ...
    public static <T> Comparator<T> kompozitum(Comparator<T> prim1, Comparator<T> prim2) {
        return new KompozitPrimerjalnik(prim1, prim2);
    }

    private static class KompozitPrimerjalnik<T> implements Comparator<T> {
        private Comparator<T> prim1;
        private Comparator<T> prim2;
        public KompozitPrimerjalnik(Comparator<T> prim1, Comparator<T> prim2) {
            this.prim1 = prim1;
            this.prim2 = prim2;
        }
        @Override
        public int compare(T a, T b) {
            int prva = prim1.compare(a, b);
            if(prva != 0) return prva;
